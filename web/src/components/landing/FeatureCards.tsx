...
function LeaderboardMock() {
  const rows = [
    ["🥇", "Solking", "+$412.8k", "from-solis to-teal"],
  ...
  return (
    <div className="rounded-xl bg-ink/70 border border-line p-3">
      <div className="flex items-center gap-2">
        <div className="h-7 w-7 rounded-full bg-gradient-to-br from-solis to-teal" />
...
function AlertMock() {
  return (
    <div className="flex items-start gap-2.5 rounded-xl bg-ink/70 border border-line px-3 py-2.5">
      <div className="grid h-7 w-7 place-items-center rounded-lg bg-solis/15 text-base">🐳</div>
      <div className="min-w-0">
        <div className="text-sm font-semibold text-white">BONK is up 24%</div>
        <div className="text-xs text-muted">50 degens just aped $88,203</div>
      </div>
  ...
  return (
    <div className="space-y-2">
      ...
      <div className="flex items-center justify-center gap-2 rounded-xl bg-surface-2 border border-line py-2.5 text-sm font-bold text-white">
        <span className="text-solis">G</span> Sign in with Google
      </div>
    </div>
  );
}
...
function CustodyMock() {
  return (
    <div className="flex items-center gap-3 rounded-xl bg-ink/70 border border-line px-3 py-3">
      <div className="grid h-9 w-9 place-items-center rounded-xl bg-solis/15 text-lg">🔒</div>
      <div>
        <div className="text-sm font-semibold text-white">your keys</div>
        <div className="text-xs text-muted">your coins — always</div>
      </div>
      <span className="ml-auto rounded-md bg-solis/15 px-2 py-1 text-[10px] font-bold text-solis">NON-CUSTODIAL</span>
    </div>
  );
}

const FEATURES: { eyebrow: string; title: string; visual: ReactNode; span?: boolean }[] = [
  { eyebrow: "Solis Board", title: "climb the board, earn your name", visual: <LeaderboardMock /> },
    ...
    <section id="features" className="mx-auto max-w-6xl px-4 sm:px-6 py-24 sm:py-32">
      ...
      <div className="mt-12 grid sm:grid-cols-2 lg:grid-cols-3 gap-5">
        {FEATURES.map((f) => (
          <div
            key={f.eyebrow}
            className="group flex flex-col rounded-3xl border border-line bg-surface/50 p-6 transition-all duration-300 ease-out hover:-translate-y-1.5 hover:border-solis/40 hover:bg-surface hover:shadow-2xl hover:shadow-solis/10"
          >
            <span className="text-xs font-bold uppercase tracking-widest text-solis">
              {f.eyebrow}
            </span>
